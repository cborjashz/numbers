...
async def login(data: LoginRequest):
    ...
    try:
        ...
        return {
            ...
            "rol": "mayorista" if id_mayorista is None else "vendedor"
        }
...
# ===== DASHBOARD (solo para mayoristas) =====
@app.get("/api/dashboard/estadisticas")
async def dashboard_estadisticas(authorization: str = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Token no proporcionado")
    token = authorization.replace("Bearer ", "")
    id_usuario, _ = await validar_token(token)
    ...
    # Obtener id_mayorista (debe ser NULL para ser mayorista)
    cursor.execute("SELECT id_mayorista FROM usuarios WHERE id_usuario = %s", (id_usuario,))
    resultado = cursor.fetchone()
    if not resultado or resultado[0] is not None:
        conn.close()
        raise HTTPException(status_code=403, detail="Acceso denegado. Solo para mayoristas.")
    id_mayorista = id_usuario  # El mayorista es el dueño de la cuenta

    # Total ventas
    cursor.execute("SELECT COALESCE(SUM(total), 0) FROM ventas WHERE id_mayorista = %s", (id_mayorista,))
    total_ventas = cursor.fetchone()[0]

    # Total vendedores
    cursor.execute("SELECT COUNT(*) FROM usuarios WHERE id_mayorista = %s AND activo = TRUE", (id_mayorista,))
    ...
    cursor.execute("""
        ...
        WHERE id_mayorista = %s AND DATE(fecha_hora AT TIME ZONE 'UTC' AT TIME ZONE 'America/Managua') = %s
    """, (id_mayorista, hoy))
    ventas_hoy = cursor.fetchone()[0]

    # Ventas por vendedor
    cursor.execute("""
        SELECT u.nombre_usuario, COALESCE(SUM(v.total), 0) as total
        FROM usuarios u
        LEFT JOIN ventas v ON u.id_usuario = v.id_usuario
        WHERE u.id_mayorista = %s AND u.activo = TRUE
        GROUP BY u.id_usuario, u.nombre_usuario
        ORDER BY total DESC
    """, (id_mayorista,))
    ventas_por_vendedor = [{"nombre": row[0], "total": float(row[1])} for row in cursor.fetchall()]

    # Ventas por cierre
    cursor.execute("""
        SELECT cierre_asignado, COALESCE(SUM(total), 0) as total
        FROM ventas
        WHERE id_mayorista = %s
        GROUP BY cierre_asignado
    """, (id_mayorista,))
    ventas_por_cierre = [{"cierre": row[0], "total": float(row[1])} for row in cursor.fetchall()]
    ...
    return {
        ...
        "ventas_por_cierre": ventas_por_cierre
    }

@app.get("/api/dashboard/vendedores")
async def dashboard_vendedores(authorization: str = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Token no proporcionado")
    token = authorization.replace("Bearer ", "")
    id_usuario, _ = await validar_token(token)
    ...
    if not resultado or resultado[0] is not None:
        conn.close()
        raise HTTPException(status_code=403, detail="Acceso denegado. Solo para mayoristas.")
    id_mayorista = id_usuario

    cursor.execute("""
        SELECT id_usuario, nombre_usuario, limite_venta, max_sesiones, fecha_expiracion, activo
        FROM usuarios
        WHERE id_mayorista = %s
    ...
    return [{
        ...
        "nombre": row[1],
        "usuario": row[1],
        "limite_venta": float(row[2]),
        "max_sesiones": row[3],
        "fecha_expiracion": row[4].isoformat() if row[4] else None,
        "activo": row[5]
    } for row in filas]

@app.post("/api/dashboard/vendedores")
async def dashboard_crear_vendedor(data: dict, authorization: str = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Token no proporcionado")
    token = authorization.replace("Bearer ", "")
    id_usuario, _ = await validar_token(token)
    ...
    limite_venta = data.get("limite_venta")

    if not nombre or not usuario or not password or not limite_venta:
        conn.close()
        raise HTTPException(status_code=400, detail="Faltan datos obligatorios.")

    cursor.execute("""
        INSERT INTO usuarios (nombre_usuario, password_hash, limite_venta, max_sesiones, activo, id_mayorista)
        VALUES (%s, %s, %s, 1, TRUE, %s)
    """, (usuario, password, limite_venta, id_mayorista))

    conn.commit()
    conn.close()
    return {"detail": "Vendedor creado exitosamente"}

@app.put("/api/dashboard/vendedores/{id_vendedor}")
async def dashboard_editar_vendedor(id_vendedor: int, data: dict, authorization: str = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Token no proporcionado")
    token = authorization.replace("Bearer ", "")
    id_usuario, _ = await validar_token(token)
    ...
    # Verificar que el vendedor pertenece al mayorista
    cursor.execute("SELECT id_usuario FROM usuarios WHERE id_usuario = %s AND id_mayorista = %s", (id_vendedor, id_mayorista))
    if not cursor.fetchone():
        conn.close()
        raise HTTPException(status_code=404, detail="Vendedor no encontrado o no pertenece a este mayorista.")

    max_sesiones = data.get("max_sesiones")
    fecha_expiracion = data.get("fecha_expiracion")

    if max_sesiones is not None:
        cursor.execute("UPDATE usuarios SET max_sesiones = %s WHERE id_usuario = %s", (max_sesiones, id_vendedor))
    if fecha_expiracion is not None:
        if fecha_expiracion == "":
            cursor.execute("UPDATE usuarios SET fecha_expiracion = NULL WHERE id_usuario = %s", (id_vendedor,))
        else:
            cursor.execute("UPDATE usuarios SET fecha_expiracion = %s WHERE id_usuario = %s", (fecha_expiracion, id_vendedor))

    conn.commit()
    conn.close()
    return {"detail": "Vendedor actualizado exitosamente"}

@app.delete("/api/dashboard/vendedores/{id_vendedor}")
async def dashboard_desactivar_vendedor(id_vendedor: int, authorization: str = Header(None)):
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Token no proporcionado")
    token = authorization.replace("Bearer ", "")
    id_usuario, _ = await validar_token(token)
    ...
    cursor.execute("UPDATE usuarios SET activo = NOT activo WHERE id_usuario = %s", (id_vendedor,))
    conn.commit()
    conn.close()
    return {"detail": "Estado del vendedor cambiado exitosamente"}